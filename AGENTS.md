...
本项目前端作为轻量桌面应用，**所有 UI 与交互修改必须严格遵守以下 18 项核心铁律**：
...
18. **模型自动重连与多轮切换自愈铁律 (Model Failover & Multi-Cycle Switching Invariance)**：
    - **瞬态退避重连**：429/5xx/网络抖动/请求超负荷等瞬态错误按 2/4/8s 自动退避重连，上限 24 次（相同模型与 Prompt 复用）；
    - **候选池去重与智能补齐**：永久/未知错误启动模型切换；优先白名单 MRU 顺序，少于 3 个候选时自动从内核可用模型补齐备选；`_sameModel` 规范化剥离 `provider/` 前缀严格去重，杜绝原失败模型混入浪费切换名额；
