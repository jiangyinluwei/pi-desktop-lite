...
3. **右键永远是“返回上一步/回退”与三态界面流**：
   - 全域彻底禁用浏览器默认右键菜单（`contextmenu` 拦截）；
   - 右键行为统一作为“返回上一步 (Step Back)”：
     - **三态界面层级流**：`Flow 交互版 (界面3)` ➔ 右键回退至 `专注版 (界面2)` ➔ 右键回退至 `详细版 (界面1)` 并失焦；
     - **基础层回退**：详细版下优先失焦高亮组件 ➔ 关闭顶层模态框/抽屉 ➔ 清空当前输入 ➔ 触发业务回退分发。所有新功能模块必须接入 `window.__piRegisterStepBack`；
4. **AI-Agent 三种基础界面规范**：
   - **界面1：初始界面-详细版 (`detailed`)**：包含沉浸式标题栏、居中品牌 Logo 组（徽标+标题+副标题）、手绘齿轮设置按钮、输入框内部功能按钮（导入图标、清空按钮、Enter 引导）及底部草图标签；
   - **界面2：初始界面-专注版 (`focus`)**：单击输入框自动进入，仅保留居中手绘 $\pi$ Logo 徽标与纯净输入框，隐藏所有按钮与副标题；右键回退至界面1；
   - **界面3：Flow 流式交互版 (`flow`)**：回车触发，手绘 Logo 移至最左上方，主体区域展示思考过程卡片（可折叠、含步骤与耗时）与 Agent 回答卡片，输入框移至最下方并自适应拉长；右键回退至界面2；
