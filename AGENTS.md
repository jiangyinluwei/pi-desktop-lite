...
4. **AI-Agent 四大基础界面规范与独立设置页面标准**：
   ...
   - **界面4：项目设置独立全屏页面 (`settings`)**：
     ...
     - **两步式自定义通道配置**：
       - 第一步：新增/管理运营商（Provider ID、接口协议[含 `openai-responses`、`openai-completions` 等]、Base URL、API Key）；
       - 第二步：在运营商卡片内新增模型（Model ID、显示名称、Context Window、Max Tokens、Reasoning），自动映射写入 `~/.pi/agent/models.json` 并添加至“当前模型列表”。
