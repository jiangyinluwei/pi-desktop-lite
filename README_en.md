...
- 🤖 **Four-State Core AI-Agent Interface System**:
  ...
  - **State 3 (Flow Stream View `flow`)**: Collapsible reasoning chains with cascading auto-collapse, real-time tool execution logs, clean Markdown streaming responses, sticky floating question tips with turn navigation, automatic failover self-healing engine (`ModelFailoverEngine`), and one-click export to local Markdown;
