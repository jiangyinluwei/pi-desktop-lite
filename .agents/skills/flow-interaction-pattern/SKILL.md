...
flowchart TD
    ...
    Classify -->|瞬态: 429/5xx/网络超时/拥塞| Reconnect[自动退避重连 2s->4s->8s, 上限24次]
    ...
    MultiCycle -- 全部候选与轮次耗尽 ≤12次 --> Fallback[恢复原模型并渲染错误卡与自愈摘要]
```
...
- **进度胶囊**：展示「自动重连中 3/24 · 8s 后重试」或「正在自动切换至 <模型> 重试 … (第 1 轮 · 2/3)」，支持退避读秒；
