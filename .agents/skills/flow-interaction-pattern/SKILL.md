...
1. **伪思考框与无显式文本完成机制**：`thinking-start` 或新轮次启动时即插入 `Thinking (0.0s)...` 占位思维切片，100ms 读秒；若捕捉到思维链 delta 则流式刷新收起态流动预览（实时展示从右向左流动输出流，紧跟最新输出内容，输出越快流动越快）与展开态正文；若模型未输出显式思考文本而直接进入工具调用（`toolcall-delta-start` / `tool-start`），或显式 `thinking-start` 后结束，封口时保留该切片并定格显示为「Thinking (X.Xs) 已完成思考」（自动消除左侧渐隐遮罩并左对齐呈现，字迹清晰纯粹），绝不物理移除；仅在模型直接输出正文（`text-start`）且无显式思考/工具边界时，作为纯正文首 token 延迟占位卡静默移除；
...
flowchart TD
    ...
    Classify -->|瞬态/TPM: 429/5xx/网络超时/TPM exhausted| Reconnect[自动退避重连 2s->4s->8s]
    Reconnect --> SameCheck{持续同错判定窗口}
    SameCheck -->|< 120s 且同错| Reconnect
    SameCheck -->|>= 120s 持续同错| Fallback[终止任务并渲染错误卡与自愈摘要]
    Classify -->|永久/未知: 401/404/额度耗尽| Switch[按白名单 MRU 切换候选模型]
    Switch -->|无其他备选且非致命| Reconnect
    Reconnect -- 耗尽且开启升级 --> Switch
    Switch --> MultiCycle{多轮巡检轮换 最多3轮}
    MultiCycle -->|退避延时 1.5s->3s->6s| TryCandidate[切换候选并重发]
    TryCandidate -->|成功输出| Succeed[临时切换转正常并持久化置顶 MRU]
    TryCandidate -->|致命 401/404| Prune[自动剔除该候选并继续下一候选]
    MultiCycle -- 全部候选与轮次耗尽 ≤12次 --> Fallback
```

- **TPM/RPM 速率限制自愈与 120 秒同错判定窗口**：
  - 遇到 `inference tpm exhausted`、TPM/RPM 速率限制、服务繁忙等错误时判定为瞬态可自愈（TRANSIENT），后台常驻 SessionHost 子进程无需重启模型，Pi 内核基于已有会话在原进程内等待恢复；
  - 设立 120 秒同错判定时间（`sameErrorTimeoutMs: 120000`）；在持续报相同错误（涵盖一切同指纹连续报错）时保持等待并自动重试，绝不提前草率报错失败；
  - 仅当模型持续报同样的错达到 120 秒时，才真正弹出错误卡并终止任务；若在 120 秒内恢复或成功输出立即自愈继续流式生成；报错指纹变化时动态重置容忍窗口；
  - **自愈心跳与 120s 硬性超时熔断**：引擎启动 1s 周期心跳时钟，在等待退避与重发请求期间每秒动态驱动持续时间递增刷新（消除假死感）；同时心跳时钟内置 120s 硬性超时判定，独立于在途网络请求阻塞，一旦持续同错达到 120s 立即主动熔断未决尝试，弹出错误卡片并终止任务；
  - **成功后时间清零铁律**：自愈/重发成功后，必须立即清空原本的时间累计（`firstErrorTimestamp = 0`、`sameErrorCount = 0`），确保下一次发生限流时从 0 秒重新开始累计计算；
- **单模型降级保护**：当白名单中无其他候选模型且当前错误非 401/404 等致命配置错误时，自动降级进入同模型 120 秒重试自愈通道，杜绝 0 秒草率放弃；
- **候选池去重与智能补齐**：优先白名单 MRU 顺序；当有效候选少于 3 个时，自动从内核已验证可用模型中补齐；`_sameModel` 规范化消除 `provider/` 前缀及大小写差异，杜绝原失败模型混入浪费名额；
- **多轮巡检与退避延时**：打破单遍快速失败，支持多轮（默认 3 轮，全局上限 12 次），候选间与轮次间施加 1.5s~6s 退避，给服务端瞬态恢复窗口；
- **进度胶囊与系统弹窗静默**：
  - TPM/RPM 速率限制自愈等待期间，**严禁触发 Windows 原生系统弹窗 (Toast)**，也**严禁在回答区插入错误卡片**；
  - 进度胶囊作为**纯状态示意条**，精准展示「触发模型每分钟推理速率限制 (TPM/RPM)，正在等待恢复... (已持续 14s/判定 120s) · 4s 后重试」或「正在自动切换至 <模型> 重试 … (第 1 轮 · 2/3)」，支持退避读秒；重发中展示「... 正在重发请求 …」且秒数每秒实时递增跳动；
- **步骤流记录保留与首响应即时自愈流水线**：
...
- **MRU 保护**：重试期间仅临时切换（`pi_set_model`），候选模型成功输出后才持久化置顶 MRU；
- **终止守则**：用户点击「⏹ 终止」立即彻底强杀退出，全链路严禁触发任何自动重连或模型切换。
