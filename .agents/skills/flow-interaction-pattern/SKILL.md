...
本技能定义 Flow 流式交互界面（`界面3 / data-view="flow"`）的前端分层架构、步骤切片流水线、滚轮委托、轮次导航、模型无痕内置重连、文件收纳、状态隔离与会话回退规范。
...
### 5.2 手动终止绝对禁止触发重连铁律
用户点击「⏹ 终止」时，系统立即执行 `modelFailoverEngine.markTaskAborted(taskId)` 与 `cancel()`，**全链路严禁触发任何内置重连**。
...
## 📌 6. 模型无痕内置重连引擎 (ModelFailoverEngine)
