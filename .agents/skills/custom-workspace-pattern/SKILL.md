...
- **核心推理决策三大约束 (Core Constraints)**：
  - **规则 1（事实驱动与引用溯源）**：禁止凭纯记忆断言，子问题拆解 ➔ 针对性检索与多源交叉核验 ➔ 事实齐备后再推理，输出强制列出引用来源（`[标题](URL)`）；
  - **规则 2（权威度分级与 24 个月时效门禁）**：官方/标准机构/权威公告优先，自动带当前年份检索；高时效领域超过 24 个月默认过时，陈旧资料显式风险预警；
  - **规则 3（多方案人工决策暂停门禁）**：多合理方案时禁止擅自定稿，以 1 行标题 + 简述 + 优劣势 + 首选 `（推荐）`（$\le 4$ 项）暂停等待用户明确选定；
- **前置材料门禁 (Prerequisite Gate)**：首节点材料完备性校验，材料缺失输出自查清单，严禁脑补；
- **事实锁死 (Fact-Lock)**：跨表勾稽一致，严守行业合规红线；
- **开场协议**：意图嗅探 ➔ 规则 1/2 事实收集 ➔ 前置自检 ➔ 规则 3 方案确认 ➔ DAG 执行 ➔ 终审定稿与桌面直达。
...
- [ ] `AGENTS.md` 具备三大推理决策约束规则、前置门禁与数据契约；
