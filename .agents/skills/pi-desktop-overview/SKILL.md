...
- **无痕内置重连流水线 (`ModelFailoverEngine`)**：模型调用报错时隐藏「模型XXX异常」窗体，后台静默续发「继续」文本（不生成提问卡、不显示）；写死 10 次，退避 2/4/8/16s（恒封顶 16s）；轮次胶囊实时展示「自动内置重连 N/10 ...」；10 次全部耗尽才渲染错误卡并附摘要。已彻底取消自动切换模型逻辑（候选池、MRU 巡检、多轮轮转、临时 `pi_set_model` 均已移除）。手动终止绝对禁止触发重连。
