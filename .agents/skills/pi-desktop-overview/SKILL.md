...
- **内核与扩展组件管理 (`Package Catalog & Kernel Runtime`)**：
  ...
  - **Pi 内核保险与 5 次自动重连机制 (`KernelInsuranceService`)**：全局后台 Watchdog 实时监测内核状态，捕获 `crashed` 崩溃后自动触发 5 次后台平滑重连循环（1/5 ~ 5/5，1500ms 保护间隔）；自愈成功自动清零恢复正常，5 次重连均失败进入熔断并在左上角触发 **红色抖动小闪电 + 提醒文本 (`内核崩溃 (重连失败)`)**，点击直达设置页内核面板支持一键手动排查与自愈；
