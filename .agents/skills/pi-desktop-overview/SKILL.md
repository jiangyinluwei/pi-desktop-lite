...
- **模型自愈流水线 (`ModelFailoverEngine`)**：瞬态错误与“inference tpm exhausted”/TPM 速率限制 2/4/8s 自动退避重连；120 秒同错判定容忍窗口（持续 120 秒报同一错误方真正终止报错）；单模型降级保护；永久/未知错误按 MRU 候选模型多轮轮询切换（支持最多 3 轮，候选间带 1.5s~6s 平滑退避，全局上限 12 次，自动剔除 401 致命候选，支持从可用模型智能补齐）。手动终止绝对禁止触发重连。
