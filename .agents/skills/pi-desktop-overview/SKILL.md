...
- **模型自愈流水线 (`ModelFailoverEngine`)**：瞬态错误 2/4/8s 自动退避重连（上限 24 次）；永久/未知错误按 MRU 候选模型多轮轮询切换（支持最多 3 轮，候选间带 1.5s~6s 平滑退避，全局上限 12 次，自动剔除 401 致命候选，支持从可用模型智能补齐）。手动终止绝对禁止触发重连。
