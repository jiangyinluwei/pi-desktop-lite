...
- 🛠️ **高性能 Rust 后端核心**：Win32 Job Object 内核级孤儿进程收割、无内核平稳待机与全链路交互降级、无感热更新引擎、**内核保险自动重连机制**（后台检测 crashed 状态自动平滑重连最多 5 次，均失败后左上角红色抖动小闪电提醒并支持手动重启）、**Node.js 运行环境预设检测与安装拦截**（安装/更新组件与内核前自动多路径预检 Node.js/npm，缺失时优雅弹窗引导直达官网）、包市场管理、内存会话索引及全量敏感数据脱敏。
