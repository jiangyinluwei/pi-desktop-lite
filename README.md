...
- 🤖 **AI-Agent 四态核心界面系统**：
  ...
  - **界面3（Flow 流式交互版 `flow`）**：思考折叠链与自动收起、工具调用日志、纯净 Markdown 流式回答、多段对话顶部悬浮提问吸附与轮次定位导航、模型自动重连切换自愈流水线 (`ModelFailoverEngine`)、输出一键保存为桌面 Markdown；
...
- 📎 **多格式文件拖入与多模态自适应**：支持图片/文档拖拽直入、手绘微边框胶囊展示、系统绝对路径结构化注入与多模态扩展自适应建议。
- 🧠 **对话历史沉淀与业务记忆接口**：每轮生成自动持久化完整多轮快照，讯息抽屉双击即时恢复至 Flow 模式，提供标准业务记忆接口层。
...
- 🛠️ **高性能纯 Rust 后端核心**：Win32 Job Object 内核级孤儿进程收割、无感热更新引擎（流式下载 + ProgressStepper 步进）、官方包市场与队列管理、并发内存会话索引及全量数据脱敏。
