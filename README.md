...
- 🤖 **AI-Agent 四态核心界面系统**：
  - **界面1（初始界面-详细版 `detailed`）**：默认完整视图，包含沉浸标题栏、手绘齿轮设置按钮、完整输入框功能（导入图标/清空/Enter快捷引导）、**动态历史对话讯息方框抽屉**（首行常态展示 3 个，鼠标悬浮平滑渐出展开下方每行 4 个，自适应视口高度，支持 MRU 最近浏览排序、点击一键恢复对话与悬浮「×」局部隐藏）及内置自适应灵感格言跑马灯（超长文本自动从右向左无缝循环滚动）；
